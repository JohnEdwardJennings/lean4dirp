...
def applyAndAdd (f : ℕ → ℕ) (a b : ℕ) : ℕ := (f a) + b
...
/-- If we know a is true, then a is true. -/
theorem ident {a : Prop} (t : a) : a := t

/-- If a implies b and b implies c, then a implies c. -/
theorem composition {a b c : Prop} (f : a → b) (g : b → c) : a → c := g ∘ f
