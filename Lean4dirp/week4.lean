...
def map {α β : Type} (f : α → β) : L α → L β := sorry
...
-/
def filter {α : Type} (p : α → Bool) : L α → L α := sorry
...
-/
def foldl {α β : Type} (g : α → β → β) (b : β) : L α → β := sorry
...
head to the tail. -/
def foldr {α β : Type} (g : α → β → β) (b : β) : L α → β := sorry
...
-/
def reverse {α : Type} : L α → L α := sorry
...
-/
def flatten {α : Type} : L (L α) → L α := sorry
...
-/
def zip {α β : Type} : L α → L β → L (α × β) := sorry
